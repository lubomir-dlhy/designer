...
server.registerTool(
  ...
  {
    description:
      "Delete ONE file from the open project — for clearing out superseded variants (v1/v2/scratch pages) once a direction is chosen. DEFAULTS TO A DRY RUN: call with confirm=false (or omit it) to see exactly what would be deleted, then repeat with confirm=true to actually delete. There is NO undo in claude.ai/design, so by default the file's HTML is snapshotted into the session dir first and the delete is ABORTED if that snapshot fails (pass snapshot=false to override). `filename` must be the full name including extension (e.g. 'landing-v2.html'); the tool refuses with error='ambiguous' when two files share a display label, and with error='confirm-mismatch' when the product's confirm dialog names a different file than you asked for — in both cases nothing is deleted. On success, `remainingLabels` are the switcher's DISPLAY labels (extensions stripped), not filenames. Error codes split on whether a confirm click was ever dispatched. GUARANTEED NOTHING DELETED (no click was issued): 'not-found', 'ambiguous', 'confirm-mismatch', 'menu-unavailable', 'snapshot-failed', 'wrong-project', 'busy', 'switcher-unavailable', 'dialog-stuck'. AFTER a click was dispatched: 'still-present' means the file was read as still listed on consecutive checks (so it survived), while 'unverified' and 'outcome-unknown' mean the outcome could NOT be proven either way — re-run with confirm=false to see the real list before retrying, because a retry may hit an already-deleted file. A successful delete may also carry `warnings` — non-fatal cleanup problems (e.g. the stored session could not be updated) that need a human's attention even though the file is gone. Deleting a file is destructive and not idempotent: re-running after success returns error='not-found'.",
    inputSchema: {
      key: z.string().optional(),
      filename: z.string(),
