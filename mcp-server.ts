...
server.registerTool(
  ...
  {
    description:
      "Enter, inspect, or transition a claude.ai/design session. The key is the persistent project and Chrome-tab binding: use one stable, distinct key per simultaneously active design project; never reuse one key for two projects. Default action='status' is a pure read — safe to call anytime to orient without side effects. Returns stored state + currentUrl + inSession + availableFiles so you can avoid a follow-up list call. `filesScope` says what availableFiles is worth: 'visible-only' (scraped without navigating — an empty array can mean the file panel is closed, not that the project is empty), 'other-project' (the browser tab is on a different project, so the list is deliberately empty), 'raced' (the browser tab moved while the page was being read, so BOTH availableFiles and awaitingClarification were discarded — awaitingClarification is null in that case; retry), or 'not-in-session'. Use designer_list or designer_handoff when you need an authoritative list. Use this as the first tool in any agent loop.\n\nActions:\n- status (default): read-only, no mutations\n- ensure_ready: bind this key to its matching live tab, or navigate to /design if not already there\n- resume: bind the already-open matching project tab; if absent, open the stored designUrl in a new tab\n- create: new project in a new tab (requires name)\n- adopt: bind an open project tab to this key. With multiple tabs, Designer uses a unique stored key match or the exact `url`; it never closes or guesses another tab.\n- clear: dismiss blocking overlays. Verbs run this automatically; call it explicitly to recover a stuck session.",
    inputSchema: {
      ...
      url: z.string().optional().describe('For action=adopt: exact open https://claude.ai/design/p/<uuid> URL used to disambiguate multiple project tabs.'),
      fidelity: z
  ...
  async ({ key, action = 'status', name, url, fidelity }) =>
    textResult(await getController(key).session({ action, name, url, fidelity }))
);
